...
public class CommunicationCompanyGUIController extends AbstractGUIController implements HasTables {
	...
	*/
    ...
    /** The scroll pane of which to insert the controls for sending points of interest to the system */
    @FXML
    private ScrollPane scrllpnSendPI;
    
    /** The scroll pane of which to insert the controls for showing points of interest from the system */
    @FXML
    private ScrollPane scrllpnShowPIs;
	...
	*/
	...
	public PtBoolean checkDataAndSend(double hour, double minute, double second, int year, int month, int day, EtHumanKind humanKind, String phoneNumber, String latitude, String longitude, String comment){
		try {
			return comcompanyController.oeAlert(humanKind, year, month, day, (int)hour, (int)minute, (int)second, phoneNumber, latitude, longitude, comment, comment);
	...
	/**
	 * Creates the point of interest form inside the Anchor pane.
	 */
	private void createPIForm(){
		int width = 250;
		Label lblPersonType = new Label("Type of person:");
		ListView<EtHumanKind> lstvwPersonType = new ListView<EtHumanKind>();
		lstvwPersonType.setMinWidth(width);
		...
		lstvwPersonType.setMaxHeight(50);
		lstvwPersonType.setItems( FXCollections.observableArrayList( EtHumanKind.values()));
		lstvwPersonType.getSelectionModel().select(EtHumanKind.victim);
		lstvwPersonType.setOnEditCommit(new EventHandler<ListView.EditEvent<EtHumanKind>>() {
			@Override
			public void handle(EditEvent<EtHumanKind> event) {
				if (lstvwPersonType.getSelectionModel().getSelectedItems().size() > 1){
		...
		txtfldLatitude.setMaxWidth(width);
		txtfldLatitude.setPromptText("Enter a latitude");
		TextField txtarTitle = new TextField();
		txtarTitle.setMinWidth(width);
		txtarTitle.setMaxWidth(width);
		txtarTitle.setPromptText("Enter a title");
		Label lblCategory = new Label("Category:");
		ListView<EtPICategory> lstvwCategory = new ListView<EtPICategory>();
		lstvwCategory.setMinWidth(width);
		lstvwCategory.setMaxWidth(width);
		lstvwCategory.setMaxHeight(50);
		lstvwCategory.setItems( FXCollections.observableArrayList( EtPICategory.values()));
		lstvwCategory.getSelectionModel().select(EtPICategory.other);
		lstvwCategory.setOnEditCommit(new EventHandler<ListView.EditEvent<EtPICategory>>() {
			@Override
			public void handle(EditEvent<EtPICategory> event) {
				if (lstvwCategory.getSelectionModel().getSelectedItems().size() > 1){
					lstvwCategory.getSelectionModel().clearSelection();
					lstvwCategory.getSelectionModel().select(event.getNewValue());
				}
			}
		});
		Button bttnOk = new Button("Send point of interest");
		bttnOk.setDefaultButton(true);
		Button bttnClear = new Button("Reset form");
		GridPane grdpn = new GridPane();
		grdpn.add(lblPersonType, 1, 1);
		...
		grdpn.add(txtfldLatitude, 1, 6, 2, 1);
		grdpn.add(txtfldLongitude, 1, 7, 2, 1);
		grdpn.add(txtarTitle, 1, 8, 2, 1);
		grdpn.add(lblCategory, 1, 9);
		grdpn.add(lstvwCategory, 1, 10, 2, 1);
		grdpn.add(lblTimeHour, 1, 11);
		grdpn.add(sldrHourPicker, 1, 12);
		grdpn.add(txtfldCurrentSetHour, 2, 12);
		grdpn.add(lblTimeMinute, 1, 13);
		grdpn.add(sldrMinutePicker, 1, 14);
		grdpn.add(txtfldCurrentSetMinute, 2, 14);
		grdpn.add(lblTimeSecond, 1, 15);
		grdpn.add(sldrSecondPicker, 1, 16);
		grdpn.add(txtfldCurrentSetSecond, 2, 16);
		grdpn.add(bttnOk, 1, 17);
		grdpn.add(bttnClear, 2, 17);
		bttnOk.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				if (checkIfAllDialogHasBeenFilledIn(grdpn)){
					if (checkPIDataAndSend(Math.floor(sldrHourPicker.getValue()), Math.floor(sldrMinutePicker.getValue()), Math.floor(sldrSecondPicker.getValue()),
							dtpckr.getValue().getYear(), dtpckr.getValue().getMonthValue(), dtpckr.getValue().getDayOfMonth(),
							lstvwPersonType.getSelectionModel().getSelectedItem(), txtfldPhone.getText(),
							txtfldLatitude.getText(), txtfldLongitude.getText(),
							txtarTitle.getText(), lstvwCategory.getSelectionModel().getSelectedItem()).getValue())
						resetForm(grdpn);
					else
						showWarningMessage("Error", "Unable to create point of interest");
				}
				else
					showWarningNoDataEntered();
			}
		...
		bttnClear.addEventFilter(ActionEvent.ACTION, event-> resetForm(grdpn));
		scrllpnSendPI.setContent(grdpn);
	}
	 ...
	 * @return The success of the method
	 */
	public PtBoolean checkPIDataAndSend(double hour, double minute, double second, int year, int month, int day, EtHumanKind humanKind, String phoneNumber, String latitude, String longitude, String title, EtPICategory category){
		try {
			return comcompanyController.oePI(humanKind, phoneNumber, year, month, day, (int)hour, (int)minute, (int)second, latitude, longitude, title, category);
		} catch (ServerOfflineException | InvalidHumanKindException | ServerNotBoundException e) {
			showExceptionErrorMessage(e); 
		} catch (IncorrectFormatException e) {
			showWarningIncorrectInformationEntered(e);
	...
	/**
	 * Creates the request for near PIs form inside the Anchor pane
	 */
	private void showPIForm(){
		int width = 250;
		TextField txtfldPhone = new TextField();
		txtfldPhone.setMinWidth(width);
		txtfldPhone.setMaxWidth(width);
		...
		txtfldLatitude.setMaxWidth(width);
		txtfldLatitude.setPromptText("Enter a latitude");
		Button bttnOk = new Button("Show near points of interest");
		bttnOk.setDefaultButton(true);
		Button bttnClear = new Button("Reset form");
		GridPane grdpn = new GridPane();
		grdpn.add(txtfldPhone, 1, 1, 2, 1);
		grdpn.add(txtfldLatitude, 1, 2, 2, 1);
		grdpn.add(txtfldLongitude, 1, 3, 2, 1);
		grdpn.add(bttnOk, 1, 4);
		grdpn.add(bttnClear, 2, 4);
		bttnOk.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
				if (checkIfAllDialogHasBeenFilledIn(grdpn)){
					if (checkRequestPIDataAndSend(txtfldPhone.getText(), txtfldLatitude.getText(), txtfldLongitude.getText()).getValue())
						resetForm(grdpn);
					else
						showWarningMessage("Error", "Unable to create point of interest");
				}
		...
		bttnClear.addEventFilter(ActionEvent.ACTION, event-> resetForm(grdpn));
		scrllpnShowPIs.setContent(grdpn);
	}
	
	/**
	 * Checks the data is OK and if so will send it.
	 * @param phoneNumber of human who requests the data
	 * @param latitude of human
	 * @param longitude of human
	 * @return The success of the method
	 */
	public PtBoolean checkRequestPIDataAndSend(String phoneNumber, String latitude, String longitude){
		try {
			return comcompanyController.oeNearPIs(phoneNumber, latitude, longitude);
		} catch (ServerOfflineException | InvalidHumanKindException | ServerNotBoundException e) {
			showExceptionErrorMessage(e); 
		} catch (IncorrectFormatException e) {
			showWarningIncorrectInformationEntered(e);
	...
	/**
	 * Resets all the controls on the form for sending alert or PI to blank (Or similar).
	 *
